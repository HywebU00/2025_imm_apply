...
(function () {
  ...
  // 收合左欄
  document.querySelector('.close_leftblockbtn').addEventListener('click', function () {
    const leftBlock = document.querySelector('.leftBlock');
    leftBlock.classList.toggle('lefthidden');
  });
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const leftBlock = document.querySelector('.leftBlock');
    const toggleBtn = document.querySelector('.close_leftblockbtn');
    const items = document.querySelectorAll('.nodeMenu li');
    ...
    items.forEach(function (li) {
      ...
      if (subMenu) {
        ...
        subMenu.style.maxHeight = subMenu.scrollHeight + 'px'; // 展開

        // 點擊切換
        link.addEventListener('click', function (e) {
          e.preventDefault();

          const isClosed = li.classList.contains('ulclose');

          if (isClosed) {
            // 展開
            subMenu.style.maxHeight = subMenu.scrollHeight + 'px';
            li.classList.remove('ulclose');
          } else {
            // 收合
            subMenu.style.maxHeight = '0';
            li.classList.add('ulclose');
          }
        });
      }
    });
    // 頁面載入時根據視窗寬度設定左欄初始狀態（只執行一次）
    if (window.innerWidth < 991) {
      leftBlock.classList.add('lefthidden');
    } else {
      leftBlock.classList.remove('lefthidden');
    }
  });
})();
